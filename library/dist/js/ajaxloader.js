jQuery(function($) { 
	...
	var ajaxloading_id = 'loading';
	var ajaxloading_code = '少女祈禱中...';
	...
	function ajaxloadPage(url, push, getData){
		if (!ajaxisLoad){
			...
			jQuery('#' + ajaxloading_id).text(ajaxloading_code);
			jQuery('#' + ajaxcontent).fadeOut(function() {
				jQuery('#' + ajaxcontent).fadeOut(function() {
					jQuery.ajax({
						type: "GET",
						url: url,
						...
						error: function(jqXHR, textStatus, errorThrown) {
							ajaxisLoad = false;
							document.title = "爆炸了! 你请求的页面没有正确加载";
							document.getElementById(ajaxcontent).innerHTML = ajaxloading_error_code;
							jQuery('#' + ajaxloading_id).text("页面没有正确加载!,请重试!");
						}
					});
				});
			});
